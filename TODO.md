...
- [ ] **Not done — needs the user's own Render account, which I don't have access to**: applying the updated `render.yaml` blueprint (now self-contained, no manual env var step) and confirming the deploy goes green. Verify end-to-end once deployed: record -> transcribe -> appears in dashboard, on the live URL.
