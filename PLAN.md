...
Single deployable service: FastAPI serves the built React static assets, so only one process/host is needed for deployment (simpler than separate frontend/backend hosts). DB via a managed Postgres so the app host stays stateless. **Decided (Phase 7, iterated through real constraints across four options, not a single upfront choice): primary deploy is the user's own server** via `docker-compose.yml`'s `app` service — real hardware sidesteps every constraint the managed free hosts hit (RAM ceiling, shared CPU, card requirement). Evaluated Render (free/no-card, Postgres wiring and single-service serving both genuinely worked, but 512MB RAM + shared CPU too slow), Fly.io (dedicated resources would fix the speed problem, but requires a card, ruled out by the user), and Hugging Face Spaces (free tier doesn't fit this app's architecture — corrected after reading HF's actual current docs, not assumed). **`WHISPER_MODEL_SIZE` is intentionally never overridden per-host** — every target runs the same default (a real inconsistency the user caught: `render.yaml` had quietly dropped to `tiny`, since removed, and `fly.toml` explicitly pinned `small`, also removed). That default is now `medium` (bumped from `small` once real hardware was available; the user's first suggestion of a per-host override for this was explicitly rejected as reintroducing the same inconsistency). If a given host's hardware struggles with the default, that's an accepted, documented tradeoff of that host, not solved by silently swapping in a less accurate model there. Render is kept as the documented free/no-card fallback despite being slow. See `render.yaml`, `fly.toml` (kept for reference), and README.md's "Deploying" / "Hosting decision, honestly" sections.
