...
Single deployable service: FastAPI serves the built React static assets, so only one process/host is needed for deployment (simpler than separate frontend/backend hosts). DB via a free-tier managed Postgres so the app host stays stateless. **Decided (Phase 7, iterated through real constraints across three hosts, not a single upfront choice): Render**, with `WHISPER_MODEL_SIZE=tiny`. Render's own native Postgres (via `fromDatabase`) is used; Neon remains independently verified as a working alternative if a longer-lived DB is needed later (Render's free Postgres expires after 30 days). Considered Fly.io (dedicated CPU/RAM would have kept `small` viable) but it requires a card, ruled out by the user; considered Hugging Face Spaces on a mistaken assumption about its free tier, corrected after reading HF's actual current docs (free tier is Static-only or GPU-only ZeroGPU, neither fits this app). Landed back on Render, trading model accuracy (`tiny` over the app's own `small` default) for a working deploy within a genuinely free, card-free host's real constraints — see `render.yaml`, `fly.toml` (kept for reference), and README.md's "Deploying" / "Hosting decision, honestly" sections.
